...
def _refutation_plan(turn: int, cohort_size: int, top_k: int) -> int:
    ...
    def solve(current_turn: int, remaining: int) -> tuple[float, int]:
        ...
        for batch in choices:
            ...
            if current_turn == turn and remaining == max(1, cohort_size):
                root_choices.append({
                    "batch": batch, "hit_probability": batch / remaining,
                    "hit_value": hit_value, "continuation_value": continuation,
                    "miss_probability": (remaining - batch) / remaining,
    ...
    value, batch = solve(turn, max(1, cohort_size))
    emit("planner", "result", active=True, method="finite_horizon_dp",
         turn=turn, remaining_turns=max_turn + 1 - turn, cohort_size=cohort_size,
         choices=root_choices, selected_batch=batch, expected_value=value)
    return batch
...
def emit_count(turn: int, n_constraints: int, top_k: int, scores=None,
    ...
    if turn >= config.EMIT_LATE_TURN or complete:
        emit("planner", "result", active=False, method="output_gate",
             reason="Preferences exhausted" if complete else "Late-turn output gate",
             selected_batch=top_k, cohort_size=refutation_cohort_size, turn=turn)
        return top_k

    if config.GATE_MODE == "margin" and scores:
        emit("planner", "result", active=False, method="output_gate",
             reason="Score-margin gate", cohort_size=refutation_cohort_size, turn=turn)
        best = scores[0][1]
        if best <= 0:
    ...
    emit("planner", "result", active=False, method="output_gate",
         reason="Gathering evidence: use the configured safe prefix",
         cohort_size=refutation_cohort_size, turn=turn)
    if n_constraints == 0:
